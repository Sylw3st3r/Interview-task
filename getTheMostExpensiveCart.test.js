import { getPrice, getFullNameOfUser, getTheMostExpensiveCart, getTotalPriceOfCart} from "./getTheMostExpensiveCart";
...
describe("function - getPrice()",()=>{
    ...
    test("Products that price filed is of type string should be valid",()=>{
        expect(getPrice("1",[{id: 1,price: "109.95"}])).toBe(109.95);
    }),
...
describe("function - getFullNameOfUser",()=>{
    test("Check if the correct name is found",()=>{
        expect(getFullNameOfUser(1,users)).toEqual({firstname: 'tom', lastname: 'cat'});
    }),
    test("When products is an array which items don't have name field then the firstname and lastname should be undefined",()=>{
        expect(getFullNameOfUser(1,carts)).toEqual({firstname: undefined, lastname: undefined});
    }),
    test("Id passed as string should be valid",()=>{
        expect(getFullNameOfUser("1",users)).toEqual({firstname: 'tom', lastname: 'cat'});
    }),
    test("When id passed as string is equal NaN then the firstname and lastname should be undefined",()=>{
        expect(getFullNameOfUser("1sda",users)).toEqual({firstname: undefined, lastname: undefined});
    }),
    test("When id isn't neither number or string then the firstname and lastname should be undefined",()=>{
        expect(getFullNameOfUser({},users)).toEqual({firstname: undefined, lastname: undefined});
    })
})

describe("function - getTheMostExpensiveCart",()=>{
    test("Find corectly the most expensive cart",()=>{
        expect(getTheMostExpensiveCart(users,carts,products)).toEqual({cart:carts[0],user:users[0].name, totalPrice:798.04});
    })
})